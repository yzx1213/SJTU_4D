...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	else
	{
		...
		*/
		...
		else if (speedAver < 37.1 && count == 250) dirt = 1;
		if (dirt == 1) delta = 5;
		else delta = 8 + 0.0035*c.r + 160 / c.r;
		...
		if (dirt == 1) { 
			if (c.r < 30) {
				...
				expectedSpeed = 68+0.03 * c.r;
			}
		}



		double min_r = min(radius, c.r);
		if (dirt == 0) {

			if (c.r <= 90 && radius <= 90)
			{
				expectedSpeed = min_r * min_r*(-0.046) + min_r *5.3 - 54.66;
			}
			else if (c.r <= 150 && radius <= 150)
			{
				expectedSpeed = 87 + 0.08*min_r;
			}
			else if (c.r > 150 && radius <= 80)
			{
				expectedSpeed = radius * radius*(-0.046) + radius * 5.3 - 59.66;
			}
			else if (c.r > 150 && radius <= 150)
			{
				expectedSpeed = 110 + 0.03* radius;
			}

			else if (c.r < 200 && radius < 200)
			{
				expectedSpeed = 98;
			}
			else if (c.r > 200 && radius < 200)
			{
				expectedSpeed = 97 + 0.045 * radius;
			}
		...
		if (count > 100) {
			if (curSpeedErr > 0)
			{
				...
				if (fabs(*cmdSteer) < 0.25 - dirt * 0.1)
				{
					*cmdAcc = constrain(0.0, 0.45, dirt * 0.2 + kp_s * curSpeedErr + ki_s * speedErrSum + offset);
					*cmdBrake = 0;
				}
				else if (fabs(*cmdSteer) < 0.5 - dirt * 0.28)
				{
					*cmdAcc = constrain(0.0, 0.55 - dirt * 0.3, kp_s * curSpeedErr + ki_s * speedErrSum + offset);
					*cmdBrake = 0;
				}
				else
				{
					*cmdAcc = constrain(0.0, 0.3 - dirt * 0.2, kp_s * curSpeedErr + ki_s * speedErrSum + offset);
					*cmdBrake = 0;
				}
			}
			else if (curSpeedErr < 0)
			{
				*cmdBrake = constrain(0.0, 0.8, -(1 + dirt * 0.04)*kp_s * curSpeedErr / 5 - offset / 3.05);
				*cmdAcc = 0;
			}
		}
		else {
		...
		*/
		...
		if (count > 100) *cmdSteer = constrain(-1.0, 1.0, (1 - 0.05*dirt) * kp_d * D_err + ki_d * D_errSum + kd_d * D_errDiff);
		if (count < 100) {
			if (_midline[0][0] < 0)
			{
				if (fabs(offset) > 0.01) {
					*cmdSteer = constrain(-1.0, 1.0, kp_d * D_err + 1);
				}
		...
		//printf("speed:%f, offset%f\n", _speed, offset);
		//printf("radius:%f, c.r:%f, expecedspeed:%f, curspeed:%f \n", radius, c.r, expectedSpeed, _speed);
		//printf("dirt: %d \n", dirt);
		//printf("count:%d cmdSteer:%f cmdAcc:%f offset:%f\n", count, *cmdSteer, *cmdAcc, offset);
		printf("c.r:%f,radius%f,offset:%f,_speed%f \n expectedspeed%f\n", c.r, radius, offset, _speed, expectedSpeed);
		/******************************************End by Yuan Wei********************************************/
	}
}
