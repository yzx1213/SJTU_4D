...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	x = (speedErr>-2.08 && speedErr < -0.62 )?-1:-0.8*log(0.8*speedErr *speedErr + 2.1 * speedErr + 1)*speedErr / fabs(speedErr);
	float D; //= fabs(D_err) < 1.8 ? 0 : 2 * fabs(D_err);
	if (fabs(D_err) < 1.5) D = 0;
	else if (fabs(D_err) < 1.8) D = fabs(D_err);
	else D = 2 * fabs(D_err);
	S = (leaderspeed >= 70) ? (3 * log(leaderspeed) - 1.4) : 10.4;
	safe_distance = constrain(10.2, 25, x + D - int(leaderAcc*0.055) + 0.9*S);
	...
	*cmdSteer = -0.81* D_err - 1.8 * D_errDiff - 0.1*fabs(D_err)*D_err;
	//if (c.r < 120 && _midline[0][0] <= -3) *cmdSteer += 0.2;

	if (speedErr > 0) {//leader>follow 

		if (_Leader_Y > safe_distance) {
			*cmdAcc = (_Leader_Y - safe_distance) * 0.4 + 0.25* speedErr + leaderAcc * 0.02 - 0.3* fabs(D_err);
			*cmdBrake = 0;
		}

		else if (_Leader_Y < safe_distance) {
			*cmdAcc = (_Leader_Y - safe_distance) * 0.15 + leaderAcc * 0.07 + speedErr * 0.2 - 0.3* fabs(D_err);
			*cmdBrake = 0;
	...
	if (fabs(leaderAcc*0.02) < 1.5&& s > safe_distance + 2)
	{
		*cmdAcc = 1;
		*cmdBrake = 0;
	}
