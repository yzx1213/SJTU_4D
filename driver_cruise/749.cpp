...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	else
	{
		...
		*/
		...
		if (dirt == 1)  expectedSpeed = 63 + c.r*0.04;
		if (dirt == 0){
		
		if (c.r <= 90 && radius <= 90 )
		{
			expectedSpeed = c.r*c.r*(-0.046) + c.r*5.3 - 59.66 + 10;
		...
		if (count > 113) {
			if (curSpeedErr > 0)
			{
				...
				{
					*cmdAcc = constrain(0.0, 0.45, dirt * 0.13 + kp_s * curSpeedErr + ki_s * speedErrSum + offset);
					*cmdBrake = 0;
				}
		...
		*/
		...
		if (count > 113 ) *cmdSteer = constrain(-1.0, 1.0, (1 - 0.05*dirt)*kp_d * D_err + ki_d * D_errSum + kd_d * D_errDiff);
		if (count < 113) {
			if (D_err > 0)
			{
				if (fabs(offset) > 0.01) {
