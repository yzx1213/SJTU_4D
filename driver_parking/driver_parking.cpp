...
static void userDriverSetParam(bool* bFinished, float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	last_s = s;
	while (!*bFinished) {
		if (s < 1 && _speed < 0.05) *bFinished = 1;break;
		...
		else if (fabs(dist) < 0.1 && y < 0.1)
		{
			*cmdAcc = 0;*cmdBrake = 1;*cmdGear = 1;
		}
		else if (fabs(dist) < start_p && y<4) {
			flagt = 2;
			*cmdSteer = 7 * (_yaw + PI / 2) + 0.5*dist;
			if (_speed >4 * y) { *cmdBrake = 0.1 + 5 * (y - 0.1);*cmdAcc = 0; }
			else { *cmdBrake = 0;*cmdAcc = 0.2; }

			state = 5;
		}
		else if ((fabs(dist) < start_p && (fabs(dist)<0.1 || fabs(fabs(_yaw) - PI / 2) <= 0.2) && y >= 4 && flagt != 2) || flagt == 1) {
			flagt = 1;
			*cmdSteer = 5 * (_yaw + PI / 2) + 1.5*dist;
			{
				*cmdAcc = 0.02*(y - 2);
				*cmdBrake = 0;
			}
			//if (_speed > 7 * y) { *cmdBrake = 0.1 + (_speed - 7 * y)*0.1;*cmdAcc = 0; }
			//else { *cmdBrake = 0;*cmdAcc = 0.2; }
			state = 4;
		}

		else if (fabs(dist) < start_p && (fabs(dist) >= 0.1 || fabs(fabs(_yaw) - PI / 2) > 0.2) && flagt != 1) {
			*cmdSteer = 1;
		...
		else if (s < 80 && fabs(dist) < 80 && flagt != 1) {
			*cmdSteer = (_yaw - atan2(_midline[10][0] + 0.7*offset, _midline[10][1])) / 3.14;
			if (_speed > 60) { *cmdBrake = 0.1;*cmdAcc = 0; }
			else { *cmdBrake = 0;*cmdAcc = 0.2; }
			state = 2;
			pos_distance = y;
			start_p = 1.5 * a*sin(atan(a / (c.r - 5))) + a;
...
*/
	if (*bFinished)
	{
		...
		else if (_yaw<-0.8 && flag < 3)
		{
			flag = 2;
			*cmdSteer = (_speed>0?-1:0);
			*cmdBrake = 0;
			*cmdGear = 1;
			*cmdAcc = 1;
			state = 8;
			...
			flag = 3;
			*cmdSteer = (_yaw - 8 * atan2(_midline[30][0] - _width / 4, _midline[30][1])) / 3.14;
			*cmdAcc = 1;
			*cmdBrake = 0;
			*cmdGear = 1;
			state = 9;
	...
	printf("state: %d flag: %d yaw: %f y: %f\n", state, flag, _yaw, y);
