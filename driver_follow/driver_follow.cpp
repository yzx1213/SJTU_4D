...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	D_err = 0.4 * atan2(_Leader_X, _Leader_Y) + 1 * _Leader_X;//方向偏差模型
	D_errDiff = D_err - Tmp;
	Tmp = D_err;

	*cmdSteer = (-0.8 * D_err - 0.3 *D_errDiff) + 0.8*atan(4 * _Leader_X / _speed);
	...
	if (speedErr < 0 && speedErr > -20) {//头车速度小，且头车跟车速度相差不大
		...
		else if (_Leader_Y > safe_distance  && leaderAcc < -30) {//距离远 头车明显减速
			*cmdAcc = 0;
			*cmdBrake = -speedErr - 0.1 * leaderAcc;
		}

		else if (_Leader_Y > 14+ 0.01*_speed  && leaderAcc > -20) {//距离远 头车减速不明显
	...
	printf("speed %.3f Leader XY(%.3f, %.3f) D_err:%f, steer:%f, leaderspeed:%f leaderAcc:%f cmdAcc:%f brake:%f acc:%f\n", _speed, _Leader_X, _Leader_Y, D_err, *cmdSteer, leaderspeed, leaderAcc, *cmdAcc, *cmdBrake, acc);
