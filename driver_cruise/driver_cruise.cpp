...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	else
	{
		...
		*/
		...
		if (dirt == 1) delta = 5;
		else delta = 8+0.0045*c.r + 160 / c.r;
		...
		if (dirt == 1)  expectedSpeed = 63.7 + c.r*0.04;
		if (dirt == 0) {
		...
		if (count > 100) {
			if (curSpeedErr > 0)
			{
				...
				if (fabs(*cmdSteer) < 0.25 - dirt * 0.05)
				{
					*cmdAcc = constrain(0.0, 0.45, dirt * 0.14 + kp_s * curSpeedErr + ki_s * speedErrSum + offset);
					*cmdBrake = 0;
				}
			...
			{
				*cmdBrake = constrain(0.0, 0.8, -kp_s * curSpeedErr / 5 - offset / 3.05);
				*cmdAcc = 0;
			}
		}
		else {
