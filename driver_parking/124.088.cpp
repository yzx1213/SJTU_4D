...
static void userDriverSetParam(bool* bFinished, float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	float offset = s * sin(_lotAngle - atan(_carY / _carX));
	...
	if (!*bFinished) {
		...
		else if ((fabs(dist) < start_p && fabs(fabs(_yaw) - PI / 2) < 0.05 && s_diff > -0.04 && sqrt(s*s-dist*dist)<0.02) || state == 5) {
			*cmdSteer = 0;
			*cmdBrake = 1;*cmdAcc = 0;

			state = 5;
		}
		else if (fabs(dist) < start_p && fabs(fabs(_yaw) - PI / 2) < 0.05 && s > 0.03) {
			*cmdSteer = 0;
			if (_speed > 4 * s) { *cmdBrake = 0.1+(_speed - 4*s)*0.05;*cmdAcc = 0; }
	...
	if (*bFinished)
		...
		if (s < 8&&flag!=0) {
			...
			*cmdSteer = (_yaw - 8 * atan2(_midline[30][0] - _width /4, _midline[30][1]))  / 3.14-1;
			*cmdAcc = 0.8;
			*cmdGear = 1;
			flag = 0;
		}
	...
	printf("finished:%d yaw:%f,s_diff:%f,c.r:%f,pos_distance:%f,width%f\n", *bFinished,_yaw,s_diff,c.r,pos_distance,_width);
