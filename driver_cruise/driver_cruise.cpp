	...
	else
	{
		...
		*/
		...
		if (counter <= 250)
		{
			speedsum += _speed;
			counter += 1;
			averageSpeed = speedsum / counter;
		}
		if (counter > 250 && averageSpeed <= 35) dirt = 1;
		...
		*cmdSteer = constrain(-1.0, 1.0, kp_d * D_err + ki_d * D_errSum + kd_d * D_errDiff);
		...
		printf("radius:%f, c.r:%f, expecedspeed:%f, curspeed:%f, dirt:%f \n", radius, c.r, expectedSpeed, _speed,dirt);
		/******************************************End by Yuan Wei********************************************/
	}
}
