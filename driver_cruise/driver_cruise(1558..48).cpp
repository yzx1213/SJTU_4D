...
static void userDriverGetParam(float midline[200][2], float yaw, float yawrate, float speed, float acc, float width, int gearbox, float rpm) {
	...
	radius = c.r; //radius 表示前方固定距离的曲率半径
}
...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	else
	{
		...
		*/
		...
		{
			expectedSpeed = c.r*1.4;
		}
		else if (c.r > 150)
		{
			expectedSpeed = constrain(150,300,c.r*1.2);
		}
		else
		{
			expectedSpeed = 40;
		...
		*/
		...
		//get the error 
		offset = _midline[3][0];
		D_err = _yaw - 5 * atan2(_midline[3][0], _midline[3][1]) + 2*atan(20 * offset / _speed);//only track the aiming point on the middle line
